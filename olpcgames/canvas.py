...
class PygameCanvas(Gtk.Layout):
    ...
    def __init__(self, width, height):
        ...
        self._align = Gtk.Alignment.new(0.5, 0.5, 0, 0)
    ...
    def connect_game(self, app):
        ...
        # Load the modules
        # NOTE: This is delayed because pygame.init() must come after the
        # embedding is up
        if ':' not in app:
            app += ':main'
        mod_name, fn_name = app.split(':')
        self.mod_name = mod_name
        # log.info(self.mod_name)
        mod = __import__(mod_name, globals(), locals(), [])
        # print mod_name
        # mod = __import__('MainGame', globals(), locals(), [])
