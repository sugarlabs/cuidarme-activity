...
def wrap(value, tube=None, path=None):
    ...
    if isinstance(value, proxies._ProxyMethod):
        ...
        value._proxy_method = DBUSMethod(
            value._proxy_method, tube=tube, path=path)
        return value
...
class DBUSProxy(object):
    ...
    def add_signal_receiver(
        ...
        """
        log.info(
            """Setting signal receiver %s for event %s on interface %s (object path %s) with sender_keyword = %r""",
            callback,
            eventName,
            interface,
            path,
...
class DBUSMethod(object):
    ...
    def __call__(self, *args, **named):
        ...
        log.info(
            'Calling proxy for %s with *%s, **%s',
            self.__proxy,
            args,
            named)
        callback, errback = named.get(
            'reply_handler'), named.get('error_handler')
        if not callback:
            raise TypeError(
                """Require a reply_handler named argument to do any asynchronous call""")
        else:
