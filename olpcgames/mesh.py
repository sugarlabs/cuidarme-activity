...
def _getConn(activity):
    ...
    if conn:
        ...
        if hasattr(activity._shared_activity, 'telepathy_conn'):
            log.debug(
                '''new-style api for retrieving telepathy connection present''')
...
setup(sugar.activity.Activity, telepathy.client.Connection)'''
    ...
    if tubes_chan is None:
        ...
        tubes_chan = conn.request_channel(
...
def lookup_buddy(dbus_handle, callback, errback=None):
    ...
    if not errback:
        def errback(error):
            log.error(
                """Failure retrieving handle for buddy lookup: %s""",
                error)
    ...
    def with_my_csh(my_csh):
        ...
        def _withHandle(handle):
            ...
            name, path = pservice.get_preferred_connection()
            callback(
                pservice.get_buddy_by_telepathy_handle(
                    name, path, handle))
        if my_csh == cs_handle:
            conn.GetSelfHandle(
                reply_handler=_withHandle,
                error_handler=errback)
            log.debug(
                'CS handle %s belongs to me, looking up with GetSelfHandle',
                cs_handle)
...
def _get_presence_service():
    ...
    try:
        ...
        log.error(
            """Failure in _get_presence_service: %s""",
            get_traceback(err))
...
class PygameTube(ExportedGObject):
    ...
    def new_participant_cb(self, new_bus_names):
        ...
        log.debug("new participant. new bus names %s, old %s" %
                  (new_bus_names, self.ordered_bus_names))
...
def dbus_get_object(handle, path, warning=True):
    ...
    if warning:
        log.warn(
            'Use of dbus_get_object is only safe from the GTK mainloop, use dbus_get_object_proxy instead: %s %s',
            handle,
            path)
