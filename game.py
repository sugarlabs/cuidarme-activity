...
class game ():
    ...
    def __init__(self):
        ...
        # camera variables
        # absolute x,y position of the screen from the upper-left corner of the
        # level
        ...
        self.screenSize = (
            self.screenTileSize[1] *
            self.tilesX,
            self.screenTileSize[0] *
            self.tilesY)
    ...
    def SmartMoveScreen(self):
        if (Globals.thisLevel.levelSize > 1200):
            possibleScreenX = Globals.player.x - \
                ((self.screenTileSize[1] / 2) * self.tilesX)
            possibleScreenY = Globals.player.y - \
                ((self.screenTileSize[0] / 2) * self.tilesY)
            # possibleScreenY = 0
    ...
    def MoveScreen(self, xxx_todo_changeme):
        (newX, newY) = xxx_todo_changeme
        self.screenPixelPos = (newX, newY)
        # nearest-tile position of the screen from the UL corner
        self.screenNearestTilePos = (int(newY / 25), int(newX / 25))
        self.screenPixelOffset = (
            newX - self.screenNearestTilePos[1] * 25,
            newY - self.screenNearestTilePos[0] * 25)
