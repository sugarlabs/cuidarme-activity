...
class Juego():
	...
	def __init__ (self):
		...
		self.game_StarBig_On = pygame.image.load("res/sprite/StarBig_On.png").convert_alpha()
		...
		self.starIcon = pygame.image.load("res/tiles/star2.png").convert_alpha()
	...
	def drawPowerups(self):
		for item in Globals.thisLevel.powerups:
			if (item[4] == 1):
				draw(self.starIcon,(item[0]-Globals.thisGame.screenPixelPos[0], item[2]-Globals.thisGame.screenPixelPos[1]))
...
class ghost():
	...
	def Draw (self):
		...
		Globals.screen.blit (self.anim[ self.animFrame ], (self.x - Globals.thisGame.screenPixelPos[0], self.y - Globals.thisGame.screenPixelPos[1]))
	...
	def Move (self):
		...
		if not Globals.thisLevel.CheckIfHitWall((self.x+self.velX,self.y+self.velY), (50,100)) and not Globals.thisLevel.CheckIfHit((Globals.player.x+Globals.player.velX,Globals.player.y+Globals.player.velY),(self.x+self.velX,self.y+self.velY),(50,100)):
			self.x += self.velX
			self.y += self.velY
			self.timer += 1
			if (self.timer >= 60):
		...
		for line in f:
			...
			if (j == "'" or j == ""):
				...
				if firstWord == "lvlwidth":
					...
					self.thisFruit = pygame.image.load("res/sprite/"+Globals.itemTypes[self.fruitType]+".png")
